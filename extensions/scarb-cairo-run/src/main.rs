...
fn main() -> Result<()> {
    ...
    let path = Utf8PathBuf::from(env::var("SCARB_TARGET_DIR").unwrap())
        .join("dev")
        .join(filename.clone());

    if !path.exists() {
        anyhow::bail!(format!(
            "File {} does not exist. Run scarb build to compile all of this package's Cairo files.",
            filename
        ));
    }
    ...
    let sierra_program = cairo_lang_sierra::ProgramParser::new()
        ...
        .expect("Failed to parse sierra file");
